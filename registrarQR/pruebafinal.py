        ...
        while True:
            ...
            for obj in decoded_objects:
                ...
                if data in qr_registrados:
                    ...
                    # Concatena el grado, sección y turno con el nombre del archivo
                    nombre_archivo = f"{grado}_{seccion}_{turno}_Asistencia.xlsx"
                    ...
                    # Verifica si la fecha actual ya existe en la hoja de trabajo
                    if fecha_actual in [cell.value for cell in ws[1]]:
                        # Si la fecha ya existe, busca la columna correspondiente y agrega la hora
                        for i, cell in enumerate(ws[1]):
                            if cell.value == fecha_actual:
                                ws.cell(row=len(ws['A'])+0, column=i+1, value=hora_actual)
                    else:
                        # Si la fecha no existe, crea una nueva columna y agrega la hora
                        ws.cell(row=1, column=len(ws[1])+1, value=fecha_actual)
                        ws.cell(row=len(ws['A'])+2, column=len(ws[1]), value=hora_actual)

                    # Agrega los datos del estudiante a la hoja de trabajo
                    ws.append(datos)
